...
--   GRU
--
-- Pontuação: estrutura-base incluída na versão 1.1; regras oficiais
-- e critérios serão cadastrados conforme documento oficial de Goiânia.
--
-- Observação:
...
-- ============================================================
-- 15.1 COMPLEMENTOS DE MENU / CENTRAL DE ALERTAS / PARÂMETROS
-- ============================================================

-- Central de Alertas: suporta alertas gerais e específicos por usuário.
CREATE TABLE alerta (
    ...
    CONSTRAINT chk_alerta_modulo
        CHECK (modulo IN ('SISTEMA', 'FROTAS', 'FISCALIZACAO', 'RELATORIOS', 'ADMINISTRACAO')),

    CONSTRAINT chk_alerta_prioridade
        CHECK (prioridade IN ('BAIXA', 'MEDIA', 'ALTA', 'CRITICA')),

    CONSTRAINT chk_alerta_status
        CHECK (status IN ('PENDENTE', 'LIDO', 'RESOLVIDO', 'ARQUIVADO'))
);

CREATE INDEX idx_alerta_usuario_status
    ON alerta (id_usuario, status);

CREATE INDEX idx_alerta_modulo_status
    ON alerta (modulo, status);

-- Parâmetros gerais e por módulo. O valor em JSONB evita criar
-- uma tabela de configuração para cada novo parâmetro do sistema.
CREATE TABLE parametro_sistema (
    id_parametro BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    chave VARCHAR(150) NOT NULL UNIQUE,
    modulo VARCHAR(30) NOT NULL DEFAULT 'SISTEMA',
    nome VARCHAR(200) NOT NULL,
    descricao TEXT,
    valor JSONB NOT NULL,
    tipo_valor VARCHAR(20) NOT NULL DEFAULT 'JSON',
    ativo BOOLEAN NOT NULL DEFAULT TRUE,
    atualizado_por BIGINT,
    atualizado_em TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    ...
    CONSTRAINT chk_parametro_modulo
        CHECK (modulo IN ('SISTEMA', 'FROTAS', 'FISCALIZACAO', 'RELATORIOS', 'ADMINISTRACAO')),

    CONSTRAINT chk_parametro_tipo
        CHECK (tipo_valor IN ('BOOLEAN', 'INTEGER', 'NUMERIC', 'TEXT', 'JSON'))
);

-- Perfil do usuário é uma projeção dos dados já existentes, evitando
-- duplicação entre servidor, usuário, perfil e áreas de gestão.
    ...
    COALESCE(
        jsonb_agg(
            DISTINCT jsonb_build_object(
                'id_area_gestao', ag.id_area_gestao,
                'nome', ag.nome,
                'descricao', ag.descricao
            )
        ) FILTER (WHERE ag.id_area_gestao IS NOT NULL),
        '[]'::jsonb
    ) AS areas_gestao
FROM usuario u
...
LEFT JOIN usuario_area_gestao uag ON uag.id_usuario = u.id_usuario
LEFT JOIN area_gestao ag ON ag.id_area_gestao = uag.id_area_gestao
GROUP BY
...
-- ============================================================
-- 15.2 PONTUAÇÃO - ESTRUTURA-BASE
-- ============================================================
-- A tela fica restrita por permissão ao Gestor da Fiscalização.
-- Os itens, documentos e regras de cálculo serão alimentados conforme
-- o documento oficial de Goiânia, sem codificar critérios antecipadamente.

CREATE TABLE pontuacao_item (
    id_item BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    codigo VARCHAR(100) NOT NULL UNIQUE,
    nome VARCHAR(200) NOT NULL,
    descricao TEXT,
    valor_pontos NUMERIC(10,2),
    ativo BOOLEAN NOT NULL DEFAULT TRUE,
    criado_por BIGINT,
    data_criacao TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    atualizado_em TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT fk_pontuacao_item_usuario
        FOREIGN KEY (criado_por)
        REFERENCES usuario (id_usuario)
        ON DELETE SET NULL
);

CREATE TABLE pontuacao_item_documento (
    id_item_documento BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT fk_pontuacao_item_documento_item
        FOREIGN KEY (id_item)
        REFERENCES pontuacao_item (id_item)
        ON DELETE CASCADE
);

CREATE TABLE pontuacao_registro (
    id_pontuacao BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    id_item BIGINT NOT NULL,
    id_servidor BIGINT,
    id_equipe BIGINT,
    ...
    pontos_aplicados NUMERIC(10,2) NOT NULL,
    data_registro TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    registrado_por BIGINT NOT NULL,
    observacao TEXT,

    CONSTRAINT fk_pontuacao_registro_item
        FOREIGN KEY (id_item)
        REFERENCES pontuacao_item (id_item),

    CONSTRAINT fk_pontuacao_registro_servidor
        FOREIGN KEY (id_servidor)
        REFERENCES servidor (id_servidor)
        ON DELETE SET NULL,

    CONSTRAINT fk_pontuacao_registro_equipe
        FOREIGN KEY (id_equipe)
        REFERENCES equipe (id_equipe)
        ON DELETE SET NULL,

    CONSTRAINT fk_pontuacao_registro_usuario
        FOREIGN KEY (registrado_por)
        REFERENCES usuario (id_usuario),

    CONSTRAINT chk_pontuacao_destino
        CHECK (id_servidor IS NOT NULL OR id_equipe IS NOT NULL),

    CONSTRAINT chk_pontuacao_quantidade
        CHECK (quantidade > 0)
);

CREATE INDEX idx_pontuacao_registro_servidor
    ON pontuacao_registro (id_servidor, data_registro);

CREATE INDEX idx_pontuacao_registro_equipe
    ON pontuacao_registro (id_equipe, data_registro);

-- ============================================================
-- 15.3 LOGS DE ACESSO / VISÕES DE AUDITORIA
-- ============================================================
...
CREATE TABLE log_acesso (
    ...
    CONSTRAINT chk_log_acesso_evento
        CHECK (
            tipo_evento IN (
                'LOGIN',
                'LOGOUT',
...
CREATE INDEX idx_log_acesso_usuario_data
    ON log_acesso (id_usuario, data_hora DESC);

CREATE INDEX idx_log_acesso_evento_data
    ON log_acesso (tipo_evento, data_hora DESC);

-- Aba: Logs de Ações
CREATE VIEW vw_logs_acoes AS
SELECT
    a.id_auditoria,
    a.id_usuario,
    u.login,
...
-- Aba: Alterações de Registros
CREATE VIEW vw_alteracoes_registros AS
SELECT
    a.id_auditoria,
    a.id_usuario,
    u.login,
...
FROM auditoria a
JOIN usuario u ON u.id_usuario = a.id_usuario
WHERE a.dados_anteriores IS NOT NULL
   OR a.dados_novos IS NOT NULL;

-- A exportação não exige tabela própria: a tela utiliza estas consultas
-- (ou as tabelas-base) e a aplicação gera CSV/XLSX/PDF conforme necessário.

-- ============================================================
-- 15.4 AJUSTES DE ATESTAÇÃO E PERMISSÕES
-- ============================================================

ALTER TABLE atestacao
    ADD CONSTRAINT uq_atestacao_usuario_relatorio
    UNIQUE (id_relatorio, id_usuario);

INSERT INTO permissao (codigo, nome, descricao, modulo) VALUES
    ('FROTAS_GERENCIAR_SERVIDORES', 'Gerenciar servidores de Frotas',
     'Permite consultar e gerenciar servidores utilizados no módulo de Frotas', 'FROTAS'),
    ('FISCALIZACAO_GERENCIAR_VIATURAS', 'Gerenciar viaturas',
     'Permite gerenciar viaturas disponíveis para a Fiscalização', 'FISCALIZACAO'),
    ('FISCALIZACAO_GERENCIAR_PONTUACAO', 'Gerenciar pontuação',
     'Permite acessar e gerenciar itens, documentação e registros de pontuação', 'FISCALIZACAO'),
    ('ALERTAS_VISUALIZAR', 'Visualizar alertas',
     'Permite acessar a Central de Alertas', 'SISTEMA'),
    ('PERFIL_VISUALIZAR', 'Visualizar próprio perfil',
     'Permite consultar os dados do próprio perfil', 'SISTEMA'),
    ('PERFIL_EDITAR', 'Editar próprio perfil',
     'Permite editar informações permitidas do próprio perfil', 'SISTEMA'),
    ('ADMIN_GERENCIAR_PARAMETROS', 'Gerenciar parâmetros do sistema',
     'Permite alterar parâmetros e configurações do sistema', 'ADMINISTRACAO'),
    ('AUDITORIA_VISUALIZAR_ACESSOS', 'Visualizar logs de acesso',
     'Permite consultar registros de login e acesso', 'AUDITORIA'),
    ('AUDITORIA_VISUALIZAR_ACOES', 'Visualizar logs de ações',
     'Permite consultar ações realizadas no sistema', 'AUDITORIA'),
    ('AUDITORIA_VISUALIZAR_ALTERACOES', 'Visualizar alterações de registros',
     'Permite consultar dados anteriores e novos dos registros', 'AUDITORIA')
...
JOIN permissao pe ON pe.codigo IN (
    'FROTAS_GERENCIAR_SERVIDORES',
    'FISCALIZACAO_GERENCIAR_VIATURAS',
    'FISCALIZACAO_GERENCIAR_PONTUACAO',
    'ALERTAS_VISUALIZAR',
    'PERFIL_VISUALIZAR',
    'PERFIL_EDITAR',
    'ADMIN_GERENCIAR_PARAMETROS',
    'AUDITORIA_VISUALIZAR_ACESSOS',
    'AUDITORIA_VISUALIZAR_ACOES',
    'AUDITORIA_VISUALIZAR_ALTERACOES'
)
WHERE p.nome = 'Administrador'
ON CONFLICT DO NOTHING;

-- ============================================================
-- 15.5 VISÃO OPERACIONAL DA ESTRUTURA DE EQUIPES
-- ============================================================
-- Apoio Extra permanece como tabela própria por possuir ciclo operacional,
-- mas pertence conceitualmente à área/tela de Equipes.
CREATE VIEW vw_equipes_com_apoios AS
SELECT
    e.id_equipe,
    e.numero AS nome,
    e.tipo,
    e.status,
    COUNT(a.id_apoio) FILTER (WHERE a.hora_fim IS NULL) AS apoios_ativos,
    COUNT(a.id_apoio) AS total_apoios
FROM equipe e
LEFT JOIN apoio_extra a ON a.id_equipe = e.id_equipe
GROUP BY e.id_equipe, e.numero, e.tipo, e.status;
