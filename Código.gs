...
function getUsers() {
  ...
  var rows = values.slice(1).filter(function (r) { return r[0]; });
  return rows.map(function (r) {
    return { id: String(r[0]), username: String(r[1]), name: String(r[3]) };
  });
}
...
function addUser(username, password, name) {
  ...
  var values = sh.getDataRange().getValues();
  for (var i = 1; i < values.length; i++) {
    if (String(values[i][1]).toLowerCase() === username) {
      throw new Error("Já existe um usuário com esse nome.");
    }
  }
  sh.appendRow([Utilities.getUuid(), username, hashPassword_(password), (name && String(name).trim()) || username, new Date().getTime()]);
  return getUsers();
}

// Atualiza usuário/nome de um acesso já existente (usado pelo botão "Editar" na aba Usuários).
// Não mexe na senha - isso é feito por changeUserPassword().
function updateUser(id, username, name) {
  ...
  var values = sh.getDataRange().getValues();
  for (var i = 1; i < values.length; i++) {
    if (String(values[i][1]).toLowerCase() === username && String(values[i][0]) !== String(id)) {
  ...
  for (var i = 1; i < values.length; i++) {
    if (String(values[i][0]) === String(id)) {
      ...
      sh.getRange(i + 1, 4).setValue((name && String(name).trim()) || username);
      found = true;
      break;
    }
  }
...
function changeUserPassword(id, newPassword) {
  ...
  for (var i = 1; i < values.length; i++) {
    if (String(values[i][0]) === String(id)) {
      sh.getRange(i + 1, 3).setValue(hashPassword_(newPassword));
      found = true;
      break;
    }
  }
