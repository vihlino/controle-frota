...
var USER_HEADERS = ["id", "username", "passwordHash", "name", "createdAt", "cpf", "email", "phone"];
...
// ---------------- Link curto do QR Code ----------------
// O QR Code impresso no veículo sempre codifica o link COMPLETO (getVehicleQrLink monta
// esse link do mesmo jeito que getWebAppUrl + "?v=<id>") - assim ele continua funcionando
// mesmo se o encurtador estiver fora do ar. O link curto é só pra ficar mais fácil de ler
// e copiar na tela do painel (aba "Veículos"). Fica em cache nas Propriedades do Script,
// então só chama o TinyURL uma vez por veículo.
function shortenUrl_(longUrl) {
  var props = PropertiesService.getScriptProperties();
  var cacheKey = "shortlink_" + longUrl;
  ...
  try {
    ...
    if (resp.getResponseCode() === 200 && /^https?:\/\//.test(text)) {
      props.setProperty(cacheKey, text);
      return text;
    }
  } catch (err) {
    // Sem acesso ao TinyURL agora - sem problema, quem chamou usa o link completo mesmo.
  }
  return longUrl;
}
...
function getVehicleQrLink(vehicleId) {
  ...
  var longUrl = base + (base.indexOf("?") > -1 ? "&" : "?") + "v=" + encodeURIComponent(vehicleId);
  return { longUrl: longUrl, shortUrl: shortenUrl_(longUrl) };
}
...
function login(username, password) {
  ...
  for (var i = 1; i < values.length; i++) {
    if (String(values[i][1]).toLowerCase() === uname && String(values[i][2]) === hash) {
      return { ok: true, username: String(values[i][1]), name: String(values[i][3]) || String(values[i][1]) };
    }
  }
  return { ok: false };
}
